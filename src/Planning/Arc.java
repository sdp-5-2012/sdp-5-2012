...
public class Arc {
	...
	public int calculateArc(Point2D.Double robot, Point2D.Double ball, Point2D.Double target, double orientation) {
		...
		Point2D.Double intersectionPoint = findIntersectionPoint(robot, robotHelperPoint, ball, target);
		...
		//travel to point
		double distancePointRobotToIntersection = Math.sqrt(Math.pow(pointBall.x - intersectionPoint.x, 2) + Math.pow(pointBall.y - intersectionPoint.y,2));
