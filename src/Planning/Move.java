...
public class Move {
	...
	public static Position modifyPositionCoors(Robot robot, Position pos) {
		...
		pos.setCoors(((int) (Math.cos(angle)*x - Math.sin(angle)*y)), ((int) (Math.sin(angle)*x + Math.cos(angle)*y)));
		return pos;
	}
	...
	public static Position translatePoint(Robot robot, Position p) {
		...
		int y = -(p.getY()) - (-(robot.getCoors().getY())); // negative y to convert into normal cordinate system
		p.setCoors(x,y);
		return p;
	}
